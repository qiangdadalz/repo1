...
public class BaseDao<T> {
    ...
    public BaseDao(){
         ...
         */
        //this:代表当前运行的dao对象
        // 1）this.getClass():代表当前运行dao对象的Class对象，例如UserDao，此时BaseDao<User>
        Class clazz = this.getClass();
        // 2）clazz.getGenericSuperclass():得到当前dao对象的父类（参数化类型）：例如BaseDao<User>
        Type type = clazz.getGenericSuperclass();
        // 3）把父类类型强转成子类（参数化类型：ParameterizedType）
        ParameterizedType param = (ParameterizedType)type;
        // 4）param.getActualTypeArguments():得到参数化类型 上面的泛型类型列表，例如BaseDao<User>的User
